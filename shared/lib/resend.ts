...
export function buildArticlePublishEmailHtml(title: string, slug: string): string {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'https://ccip.app';
  const url = `${baseUrl}/news/${encodeURIComponent(slug)}`;

  return `
    <div style="font-family: sans-serif; max-width: 560px; margin: 0 auto;">
      <h2 style="color: #1e3a5f;">New Campus News Article</h2>
      <p style="color: #374151; font-size: 16px; line-height: 1.5;">
        A new article has been published in Campus News:
      </p>
      <p style="color: #111827; font-size: 18px; font-weight: 600;">
        ${escapeHtml(title)}
      </p>
